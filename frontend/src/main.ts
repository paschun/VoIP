...
app.config.errorHandler = (err, _instance, info) => { console.error(`vue global error handler [${info}]:`, err) }
...
window.addEventListener('unhandledrejection', (event) => { console.error('window.unhandledrejection', event.reason, event.promise) })
