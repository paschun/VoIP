...
export default defineComponent({
  ...
  components: { VueTagsInput, LoadingSpinner, ContactPicker },
  emits: ['sent'],
  setup() {
  ...
  data(): {
    ...
    touched: { recipients: boolean; message: boolean } // mimics regle
    ...
    return {
      ...
      tagInput: '',
      composeMessage: '',
      touched: { recipients: false, message: false },
      uploadedImages: [],
      selectedFileNames: '',
    }
  },
  ...
  methods: {
    ...
    addRecipient(number: string) {
      this.touched.recipients = true
      if (!this.recipients.includes(number)) this.recipients.push(number)
    },
    async onFilesPick(e: Event) {
      const target = e.target as HTMLInputElement
      if (!target.files) return
      this.selectedFileNames = [...target.files].map((f) => f.name).join()
      this.uploadedImages.push(...(await uploadMediaFiles(target.files, this.userStore.token)))
    },
